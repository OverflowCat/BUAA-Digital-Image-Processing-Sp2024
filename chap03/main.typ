...
#set page(paper: "iso-b5", margin: 1.8cm)
...
== 4、（b）证明只有在灰度不丢失的情况下，式（3.3-9）表示的离散直方图反变换才满足3.3.1节中的条件（a′）和（b）。（教材P121页，第3.10题。）
...
== 5、在给定应用中，一个均值模板被用于输入图像以减少噪声，然后再用一个拉普拉斯模板来增强图像中的细节。如果交换一下这两个步骤的顺序，结果是否会相同？
...
== 6、使用式（3.6-6）给出的拉普拉斯定义，证明从一幅图像中减去相应的拉普拉斯图像等同于对图像进行非锐化模板处理。
