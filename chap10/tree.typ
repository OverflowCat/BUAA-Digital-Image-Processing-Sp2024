#let tree(label, ..children) = context block(align(center, {
  let label = rect(align(center + horizon)[#label])
  let label_dim = measure(label)
  let children_widths = children.pos().map(x => measure(x).width)
  let all_children = stack(dir: ltr, spacing: 1em, ..children.pos())
  let all_children_dim = measure(all_children)
